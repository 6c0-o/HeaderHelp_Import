...
import { maximumLength, addSpace} from './utils/calculator.js'
import { ArgsNotFound, InternalError } from './utils/errorHandler.js'
...
export class Header {
    ...
    print(){
        ...
        //
        /** @var {number} */
        var maxLength = maximumLength(splitLogo);
        ...
        if (this.littleTitle){
            ...
            littleBannerTitle = addSpace(this.title, spaceNumber).toUpperCase()
    ...
    static OptionsVerifications(options, ...args){
        ...
        if(args)
        {
            for(let i = 0; i < args.length; i++){
                if(args[i].name === undefined) throw new ArgsNotFound('name', i, args[i]);
                if(args[i].info === undefined) throw new ArgsNotFound('information', i, args[i]);
            }
        }
    }
